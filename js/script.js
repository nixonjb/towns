...
if(Modernizr.webgl) {
	...
	d3.queue()
		.defer(d3.csv, "data/datat.csv")
		.defer(d3.json, "data/config.json")
		.defer(d3.json, "data/geogt.json")
	...
	function ready (error, data, config, geog){
		...
		//get column name
		console.log(dvc.map.mapVarName)
		varnames = [];
		for (var column in data[0]) {
			if (column == 'AREACD') continue;
			if (column == 'AREANM') continue;
			if (column == 'Shape__Are') continue;
			if (column == 'Shape__Len') continue;
...
//console.log(varnames)
	...
	//	selectlist(data);
		...
		data.forEach(function(d) { countById[d.AREACD] = +eval("d." + dvc.map.mapVarName);
...
//console.log(dvc.map.varcolour,dvc.map.numberBreaks);
...
console.log(defaultData);
		...
		stack = d3.stack().keys(defaultData.map(function(d) {return d[dvc.bar.stackVar]}));
				...
				// create default bars
				barg = g.append('g').selectAll('group')
				...
				barg.selectAll('rect')
					...
					.attr("width", function(d) { console.log(d, x(0));
						return x(d[1] - d[0]) })
					.attr('fill', '#206094');
				...
				// console.log(graphic_data, contextLines);
				//
